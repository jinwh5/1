        ...
        if (checkInTimeStr != null && !checkInTimeStr.isEmpty()) {
            try {
                ...
                if (!checkInTimeStr.contains(":")) {
                    checkInTimeStr += ":00"; // 添加秒
                } else if (checkInTimeStr.split(":").length < 3) {
                    checkInTimeStr += ":00"; // 添加秒
                }
                
                LocalDateTime checkInTime = LocalDateTime.parse(checkInTimeStr.replace(" ", "T"), 
                        java.time.format.DateTimeFormatter.ISO_LOCAL_DATE_TIME);
                attendance.setCheckInTime(checkInTime);
            } catch (Exception e) {
                redirectAttributes.addFlashAttribute("error", "签到时间格式无效: " + checkInTimeStr);
        ...
        if (checkOutTimeStr != null && !checkOutTimeStr.isEmpty()) {
            try {
                ...
                if (!checkOutTimeStr.contains(":")) {
                    checkOutTimeStr += ":00"; // 添加秒
                } else if (checkOutTimeStr.split(":").length < 3) {
                    checkOutTimeStr += ":00"; // 添加秒
                }
                
                LocalDateTime checkOutTime = LocalDateTime.parse(checkOutTimeStr.replace(" ", "T"), 
                        java.time.format.DateTimeFormatter.ISO_LOCAL_DATE_TIME);
                attendance.setCheckOutTime(checkOutTime);
            } catch (Exception e) {
                redirectAttributes.addFlashAttribute("error", "签退时间格式无效: " + checkOutTimeStr);
                return "redirect:/attendance/add";
            }
        }
