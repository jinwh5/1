...
public class AttendanceController {
    ...
    public String listAttendance(
        ...
        // 为每条记录提前查找工人姓名
        List<Attendance> attendanceRecords = attendancePage.getContent();
        for (Attendance attendance : attendanceRecords) {
            Long attendanceWorkerId = attendance.getWorkerId();
            // 预加载该考勤记录对应的工人信息
            if (attendanceWorkerId != null) {
                try {
                    Worker worker = workerService.getWorkerById(attendanceWorkerId);
                    if (worker != null) {
                        // 可以在这里进行一些处理，比如记录已找到的工人
                    }
                } catch (Exception e) {
                    // 忽略找不到工人的异常
                }
