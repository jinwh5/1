...
public class SafetyRecordServiceImpl implements SafetyRecordService {
    ...
    public long getSafetyDays() {
        List<SafetyRecord> records = safetyRecordRepository.findAll();
        if (records.isEmpty()) {
            return 0;
        }

        // 找到最早的记录时间
        LocalDateTime earliestRecord = records.stream()
                .map(SafetyRecord::getOccurrenceTime)
                .min(LocalDateTime::compareTo)
                .orElse(LocalDateTime.now());

        // 计算从最早记录到现在的时间差（天数）
        return ChronoUnit.DAYS.between(earliestRecord, LocalDateTime.now());
    }

    @Override
    public long getIncidentCountBySeverity(String severityLevel) {
        return safetyRecordRepository.findAll().stream()
                .filter(record -> record.getSeverityLevel().equals(severityLevel))
                .count();
    }

    @Override
    public List<SafetyRecord> search(
            Long workerId,
            Long projectId,
            String eventType,
            String severityLevel,
