...
public class DataInitializer {
    ...
    private void initializeProgress() {
        if (progressService.getAllProgress().isEmpty()) {
            // 创建示例进度数据
            Progress progress1 = new Progress();
            progress1.setProjectId(1L);
            progress1.setSection("郑州-开封段A区");
            progress1.setPlannedProgress(50);
            progress1.setActualProgress(45);
            progress1.setStartDate(LocalDate.of(2024, 1, 1));
            progress1.setPlannedEndDate(LocalDate.of(2024, 6, 30));
            progress1.setStatus("进行中");
            progress1.setDescription("路基施工完成45%");
            progress1.setObstacles("近期降雨频繁，影响施工进度");
            progress1.setSolutions("增加施工人员，延长工作时间");
            progress1.setUpdateTime(LocalDateTime.now());
            progress1.setUpdatedBy("张三");
            progress1.setRemarks("需要加强天气监测");
            progressService.saveProgress(progress1);

            Progress progress2 = new Progress();
            progress2.setProjectId(1L);
            progress2.setSection("郑州-开封段B区");
            progress2.setPlannedProgress(30);
            progress2.setActualProgress(25);
            progress2.setStartDate(LocalDate.of(2024, 2, 1));
            progress2.setPlannedEndDate(LocalDate.of(2024, 7, 31));
            progress2.setStatus("进行中");
            progress2.setDescription("路面施工完成25%");
            progress2.setObstacles("材料供应不及时");
            progress2.setSolutions("提前采购材料，建立材料储备");
            progress2.setUpdateTime(LocalDateTime.now());
            progress2.setUpdatedBy("李四");
            progress2.setRemarks("需要优化供应链管理");
            progressService.saveProgress(progress2);

            Progress progress3 = new Progress();
            progress3.setProjectId(2L);
            progress3.setSection("郑州地铁6号线A区");
            progress3.setPlannedProgress(40);
            progress3.setActualProgress(35);
            progress3.setStartDate(LocalDate.of(2024, 3, 1));
            progress3.setPlannedEndDate(LocalDate.of(2024, 8, 31));
            progress3.setStatus("进行中");
            progress3.setDescription("隧道施工完成35%");
            progress3.setObstacles("地质条件复杂");
            progress3.setSolutions("调整施工方案，增加支护措施");
            progress3.setUpdateTime(LocalDateTime.now());
            progress3.setUpdatedBy("王五");
            progress3.setRemarks("需要加强地质监测");
            progressService.saveProgress(progress3);
        }
    }
